...
| `lib/bake-light.ts`, `bake-light-core.ts`, `bake-light-worker.ts` | bakes sun and sky light per vertex as `_LIGHT` by ray casting a BVH, split across worker threads over shared memory; `lightKey` names the setup so a change rebakes everything              | `bakeLight`, `DEFAULT_LIGHT`, `lightKey`, `sunDirection`          |
...
| `lib/optimize.ts`                                                 | alpha modes, dedup/join/weld, bakes lighting, meshopt; reports triangles and measured size                                                                                                  | `optimizeGlb`, `GlbStats`                                         |
...
| `three/prepareModel.ts`                    | GLTF scene -> baked-light materials (or Lambert for a model without `_light`), nearest-neighbour textures, bounds, dispose                            |
| `three/bakedLightMaterial.ts`              | the shader-style look: warm sun and cool sky from the baked `_light` attribute, filmic curve, glowing blocks; `LIGHT` holds the shared colours        |
...
- Verified against real Mineways 13 exports up to 2.4M triangles: hundreds of materials
  collapse to 2-5 draw calls. See `models-src/README.md`.
- Lighting is baked, never computed live: builds run to millions of triangles and have to
  stay light on phones. Convert bakes it into every model (`_LIGHT`, two bytes per vertex)
  and records `light: lightKey()` in the manifest; `isUpToDate` rebakes any build whose
